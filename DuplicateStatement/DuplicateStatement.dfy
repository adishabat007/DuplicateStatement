...
class VariablesSSA {
	...
	method getVariableInRegularForm(vSSA: Variable)  returns (v : Variable)
		requires existsVSSA(vSSA)
	{
		v :| v in m && vSSA in m[v]; 
	}

	predicate existsVSSA(vSSA: Variable)
		reads this
	{
	...
	method instancesToVariables(instances: seq<Variable>) returns (V : seq<Variable>)
		requires forall i :: i in instances ==> existsVSSA(i)
		ensures |instances| == |V|
	...
	method getInstancesOfVaribleSeq(V : seq<Variable>) returns (instances : seq<Variable>)
		requires forall v :: v in V ==> v in m && |m[v]| >= 1
	{
		if V == [] { instances := []; }
		else {
...
method InstancesSetToSeq(instancesOfX: set<Variable>, X: seq<Variable>, vsSSA: VariablesSSA) returns (instancesOfXSeq: seq<Variable>)
	requires vsSSA != null
	requires instancesOfX != {}
	requires forall x :: x in X ==> x in vsSSA.m && |vsSSA.m[x]| >= 1
{
	...
	
{
	...
	////////////////////////////////////////
	//assert forall v :: v in X4Seq ==> v in vsSSA.m && |vsSSA.m[v]| >= 1;
...
method OrganizeVariables(vars1: seq<Variable>, vars2: seq<Variable>, vsSSA: VariablesSSA) returns (res: seq<Variable>)
	requires vsSSA != null
	requires forall i :: i in vars1 ==> vsSSA.existsVSSA(i)
	requires forall i :: i in vars2 ==> vsSSA.existsVSSA(i)
	...
	requires vsSSA != null
{
	...
	temp := setToSeq(XL3i);
	var S1' := ToSSA(S1, X, liveOnEntryX, (temp + XL4t + XL5t), Y, XLs', vsSSA);
	var XLs'' := XLs' + (glob(S1') - Y);
	temp := setToSeq(XL3i);
	var S2' := ToSSA(S2, X, liveOnEntryX, (temp + XL4e + XL5e), Y, XLs'', vsSSA);
	...
	requires vsSSA != null
{
	...
	temp := setToSeq(XL1i);
	var temp1 := setToSeq(XL1i);
	var S' := ToSSA(S, X, (temp + XL2Seq + XL3iSeq + XL4fSeq), (temp1 + XL2bSeq + XL3iSeq + XL4bSeq), Y, XLs', vsSSA);
...
method AssignmentFromSSA(LHS: seq<Variable>, RHS: seq<Expression>, XLs: set<Variable>, X: seq<Variable>, XLi: seq<Variable>, XLf: seq<Variable>, Y: set<Variable>, vsSSA: VariablesSSA) returns (S: Statement)
	requires vsSSA != null
	requires Valid(Assignment(LHS,RHS))
{
	// defined in thesis:
